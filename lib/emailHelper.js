...
export async function sendNotificationToUsers(userIds, subject, content) {
    ...
    try {
            ...
            if (user.email) {
                try {
                    ...
                    if (sent) {
                        ...
                        // Log Unknown Error (if sendEmail returned false without throwing)
                        await supabase.from('email_logs').insert({
                            recipient_email: user.email,
                            subject: subject,
                            status: 'Error',
                            error_message: 'Unknown error (returned false)'
                        });
                    }
                } catch (err) {
        ...
        return { success: true, message: `Sent ${sentCount} email(s).`, sentCount };
    } catch (e) {
