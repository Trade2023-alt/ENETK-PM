#!/usr/bin/env python3
"""
Create Enhanced Excel with File Import
Creates an Excel file with VBA macros for quote generation and file import functionality
"""
...
def create_enhanced_excel():
    """Create Excel file with VBA macros and file import functionality"""
    ...
    # Header
    ws['A1'] = 'QUOTE GENERATOR WITH FILE IMPORT'
    ...
    instructions = [
        '1. Use FILE IMPORT section to import data from .xlsx, .rtf, or .xml files',
        '2. OR manually fill in the quote details below',
        '3. Add line items in the table (up to 10 items)',
        '4. Click "GENERATE QUOTE" button to create a new worksheet',
    ...
    ws['B10'] = 'Click "BROWSE" to select file'
    ...
    ws['D14'] = 'IMPORT DATA'
    ws['D14'].font = button_font
    ws['D14'].fill = button_fill
    ws['D14'].alignment = Alignment(horizontal='center', vertical='center')
    ws['D14'].border = thick_border
    ...
    # Save the workbook
    wb.save('Quote_Generator_Enhanced.xlsx')
    print("Enhanced Excel workbook created: Quote_Generator_Enhanced.xlsx")
    
    # Create enhanced VBA code
    create_enhanced_vba_code()

def create_enhanced_vba_code():
    """Create enhanced VBA code with file import functionality"""
    
    vba_code = '''Sub BrowseFiles()
    ...
    If fileDialog.Show = -1 Then
        ...
        ' Update status
        Range("B12").Value = "File selected - Ready to import"
...
Sub ImportData()
    ...
    ' Get selected file
    filePath = Range("B10").Value
    fileName = Dir(filePath)
    
    ' Check if file is selected
    If filePath = "" Or filePath = "Click ""BROWSE"" to select file" Then
        MsgBox "Please select a file first using the BROWSE button.", vbExclamation, "No File Selected"
        Exit Sub
    End If
    
    ' Check if file exists
    If Dir(filePath) = "" Then
        MsgBox "Selected file does not exist. Please select a valid file.", vbExclamation, "File Not Found"
        Exit Sub
    End If
    ...
    Select Case fileExtension
        ...
        Case ".RTF"
            ImportRTFFile filePath
        Case ".XML"
            ImportXMLFile filePath
        Case Else
            MsgBox "Unsupported file type. Please select .xlsx, .rtf, or .xml file.", vbExclamation, "Unsupported File Type"
    End Select
End Sub

Sub ImportExcelFile(filePath As String)
    ...
    ' Update status
    Range("B12").Value = "Excel file imported successfully - " & itemCount & " items"
    Range("B12").Interior.Color = RGB(200, 255, 200) ' Light green
...
ErrorHandler:
    ...
    Range("B12").Value = "Import failed - " & Err.Description
    Range("B12").Interior.Color = RGB(255, 200, 200) ' Light red
End Sub

Sub ImportRTFFile(filePath As String)
    ...
    ' Update status
    Range("B12").Value = "RTF file imported - " & itemCount & " items (basic parsing)"
...
Sub ImportXMLFile(filePath As String)
    ...
    ' Update status
    Range("B12").Value = "XML file imported - " & itemCount & " items"
    Range("B12").Interior.Color = RGB(200, 255, 200) ' Light green
...
Sub ClearForm()
    ...
    Range("B10").Value = "Click ""BROWSE"" to select file"
    Range("B11").Value = "Auto-detected"
    Range("B12").Value = "Ready to import"
...
End Sub'''
    ...
    with open('Enhanced_VBA_Code.txt', 'w') as f:
        f.write(vba_code)
    
    print("Enhanced VBA code saved to: Enhanced_VBA_Code.txt")
    print("\nTo add VBA macros to your Excel file:")
    print("1. Open Quote_Generator_Enhanced.xlsx in Excel")
    print("2. Press Alt+F11 to open VBA Editor")
    print("3. Right-click on 'VBAProject' → Insert → Module")
    print("4. Copy the code from Enhanced_VBA_Code.txt and paste it")
    print("5. Save the file and close VBA Editor")
    print("6. You can now use all the buttons including file import!")
